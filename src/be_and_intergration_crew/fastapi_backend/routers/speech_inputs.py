...
def read_speech_inputs(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    inputs = db.query(SpeechInput).offset(skip).limit(limit).all()
    return inputs

@router.post("/speech-inputs/", response_model=SpeechInputSchema)
def create_speech_input(speech_input: SpeechInputSchema, db: Session = Depends(get_db)):
...
def read_speech_input(input_id: str, db: Session = Depends(get_db)):
...
def read_user_speech_inputs(user_id: str, db: Session = Depends(get_db)):
...
def read_conversation_speech_inputs(conversation_id: str, db: Session = Depends(get_db)):
...
def update_speech_input(input_id: str, speech_input: SpeechInputSchema, db: Session = Depends(get_db)):
...
def delete_speech_input(input_id: str, db: Session = Depends(get_db)):
    speech_input = db.query(SpeechInput).filter(SpeechInput.id == input_id).first()
    if speech_input is None:
        raise HTTPException(status_code=404, detail="Speech input not found")
